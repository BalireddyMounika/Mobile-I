          ...
          Container(
            child:Column(
              children: [
                Row(
                  children: [
                    InkWell(child: const Icon(Icons.arrow_back_ios,size: 25.0,),onTap:(){Get.back();},),
                    const Center(child:Text("Events",style:AppStyles.tsSecondaryRegular700,)),
                  ],),
                CustomEventStepper(),
              ],
            ),
  ...
  Widget build(BuildContext context) {
    return ListView.builder(
        ...
        itemBuilder:(BuildContext context,index){
          return Column(
            children: [
              Obx(() => controller.selectedEventStartDate.value!=null && controller.selectedEventEndDate.value!=null?
              Text("${controller.getDaysInBetween()}",style:AppStyles.tsBlackMedium14,)
             ...
             const CommonEventTitle(text: 'Schedule'),
              Row(
                children: [
                  Container(
                    margin:const EdgeInsets.only(left:Dimens.scaleX5,top:Dimens.scaleX3),
                    ...
                    child:Row(
                      ...
                      children: [
                        Obx(() {
                          final formattedStartTime = controller.getFormattedStartTime();
                          return Text(formattedStartTime);
                        }),
                        InkWell(child: const Icon(Icons.access_time_outlined),onTap: (){
                  ...
                  const Row(
                    children: [
                      Padding(
                          ...
                          child: Text("Start Time",style:AppStyles.tsSecondaryRegular700,)
                      )
                    ],
                  )
                ],
                  ...
                  children: [
                    Container(
                      ...
                      child:Row(
                        ...
                        children: [
                          Obx(() {
                            final formattedScheduleTime = controller.getFormattedScheduleTime();
                    ...
                    const Padding(padding:EdgeInsets.only(left:Dimens.paddingX5),
                      child:Text("End Time",style:AppStyles.tsSecondaryRegular700,),
                    )
                  ],
                ),
              ),
              Padding(
                ...
                child: CommonInputField(wrapper:controller.eventPlaceTextController,hintText: 'Type your description',),
              ),

              Padding(
                padding: const EdgeInsets.only(top:Dimens.paddingX3,right:Dimens.paddingX5),
                child: Align(alignment:Alignment.centerRight,
                    child:InkWell(child:
                    const Text("+ Add Schedule Fields",style:TextStyle(color:Colors.blue,decoration:TextDecoration.underline)),
                      onTap:(){
                        controller.onAddEventTimeList(
                            controller.getFormattedScheduleTime(),
                            controller.getFormattedStartTime(),
                            controller.eventPlaceTextController.controller.text
                        );
              ...
              Obx(() =>  controller.eventTimeListItems.isEmpty ?const Text(''):
              Column(
                children:List.generate(controller.eventTimeListItems.length, (index){
                  return Column(
                    children: [
                      ListTile(
                        title:Row(
                          children: [
                            ...
                            const Padding(
                              padding: EdgeInsets.only(left:Dimens.paddingX2,right:Dimens.paddingX2),
                              child: Text("to"),
                            ),
                            Text(controller.eventTimeList[index])
                          ],
                        ),
                        subtitle:Text(controller.eventTimePlaceList[index]),
                        trailing: InkWell(child: const Icon(Icons.cancel_outlined),
                          onTap: (){
